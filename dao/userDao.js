...
module.exports = {
	...
	update: function (req, res, next) {
		...
		var param = req.body;
		if(param.name == null || param.age == null || param.u_id == null) {
			jsonWrite(res, undefined);
			return;
		}
 
		pool.getConnection(function(err, connection) {
			connection.query($sql.update, [param.name, param.age, +param.u_id], function(err, result) {
				// 使用页面进行跳转提示
				if(result.affectedRows > 0) {
					res.render('suc', {
						result: result
	...
	queryById: function (req, res, next) {
		// var id = +req.query.u_id; // 为了拼凑正确的sql语句，这里要转下整数
		pool.getConnection(function(err, connection) {
